...
"""
    ...
    def setup_service_file(self):
        ...
        if not os.path.exists(self.SERV_PATH):
            ...
            logging.info("[BettercapFixer] Created Bettercap service file at /etc/systemd/system/bettercap.service")
        else:
            logging.info("[BettercapFixer] Bettercap service file already exists.")
    ...
    def _check_for_api_unavailability(self, log_message):
        ...
        if "waiting for bettercap API to be available" in log_message:
            ...
            logging.warning(f"[BettercapFixer] Detected Bettercap API unavailability ({self.api_unavailable_count}/{self.api_timeout_limit}).")
            if self.api_unavailable_count >= self.api_timeout_limit:
                logging.warning("[BettercapFixer] Bettercap API has been unavailable for too long. Restarting Bettercap...")
                self._restart_bettercap()
        else:
            self.api_unavailable_count = 0  # Reset the counter if the API is available

    def _restart_bettercap(self):
        logging.info("[BettercapFixer] Restarting Bettercap service...")
    ...
    def _schedule_restart(self):
        ...
        logging.info(f"[BettercapFixer] Scheduled a restart in {self.restart_interval} seconds.")

    def on_internet_available(self, agent):
        logging.info("[BettercapFixer] Internet available, resetting error count.")
    ...
    def on_bettercap_connection_error(self, log_message):
        if self._check_for_errors(log_message):
            self.error_count += 1
            logging.warning(f"[BettercapFixer] Detected Bettercap connection error ({self.error_count}/{self.max_errors}).")
            
            if self.error_count >= self.max_errors:
                logging.warning(f"[BettercapFixer] Maximum error threshold reached. Restarting Bettercap.")
                self._restart_bettercap()
            else:
